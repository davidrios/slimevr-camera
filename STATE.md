...
0a. Exp 05 quick test done (`experiments/05-session-bias/`): within a pose family the detector bias is nearly constant (~2° sd) → learnable; but a per-session constant/yaw table does not transfer across poses → needs a **pose-conditioned** correction model (global fine-tuning + per-session refinement in the trusted post-reset window). Next: design that model; test 'correct in poses seen during the trusted window' (reset pose / habitual idle stance).
0b. **Design 'automatic full reset' (D33):** (i) reset-pose detector from camera keypoints (template learned during the manual reset + user's idle stances); (ii) in that pose measure pelvis/chest/feet headings and bone directions (3-DoF, D31); (iii) apply through the full-reset path with the same pose assumption; (iv) confidence → prompt user if needed. Extend harness/evaluation accordingly; evaluate on MoVi still stances and on our own recordings (recurring poses).
