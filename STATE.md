...
0b. **Design 'automatic full reset' (D33):** (i) familiar-pose detector from camera keypoints (templates: the standing reset pose + the user's seated/idle stances, learned while trusted); (ii) in that pose measure pelvis/chest/feet headings and bone directions (3-DoF, D31); (iii) apply through the full-reset path with the same pose assumption; (iv) confidence → prompt user if needed. Extend harness/evaluation accordingly; evaluate on MoVi still stances and on our own recordings (recurring poses).
