...
def run(active_s, seed, duration=1200.0, fps=30.0, noise=3.0, plot=None):
    ...
    for name in TRACKER_BONES:
        ...
        rng.standard_normal(len(world[name])); rng.standard_normal(len(world[name]))
    cams = default_rig()
...
if __name__ == "__main__":
    ...
    for label, act in cadences.items():
        for seed in range(a.seeds):
            r = run(act, seed, plot=a.out / f"cadence{label}_seed{seed}.png" if seed == 0 else None)
