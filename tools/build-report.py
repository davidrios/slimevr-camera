...
<div class="finding"><b>Frontal standing familiar poses survive the loss of the second camera: 1.6–5.3° calibrated MAE on simple standing idles — inside the 5° budget, comparable to two cameras.</b> Seated poses currently fail monocular: near-horizontal bones depth-flip by ~180° (a sign-of-depth ambiguity, not noise), and part of the seated chest/hip failure is a protocol artifact — MoVi has no genuinely recurring poses, so the seated bias gets calibrated in the wrong phase of the motion. The side view fails before measurement is attempted: the pose gate's descriptor collapses when the hip line points into the camera. Two mitigations remain open — a torso-scaled, view-robust pose descriptor and an explicit view-quality gate (the lifted pose itself reveals how much of the body's lateral axis the camera actually sees). The decisive seated test is the own-room recording, which now needs only one camera.</div>
<p><b>The IMU prior defuses the depth-flip failure (experiment 09b).</b> The trackers themselves are a prior nobody else has: drifted, but within a few degrees — while a monocular depth-flip moves a heading by ~180°-class amounts. Letting the measured heading and its depth-mirrored hypothesis compete for closeness to a simulated drifted IMU heading halves the leg error (shins 51–60° → 24° MAE), and the result is identical whether the simulated drift is 1° or 5° — even a badly drifted prior separates the hypotheses.</p>
<div class="finding"><b>The same prior gives an agreement gate, and that changes the product's failure mode.</b> Applying a correction only when the bias-corrected camera heading agrees with the IMU within 10° keeps 38% of correction opportunities at <b>3.8° torso / 4.7° legs MAE</b> (worst kept 13°), stable across drift levels. The camera <em>declines</em> rather than mis-corrects: the worst case is no automatic reset — today's status quo — never a bad one. Opportunistic 30–50% coverage of matched windows is plenty for the familiar-pose product.</div>
