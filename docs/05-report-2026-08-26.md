...
## 4f. One camera instead of two (D36, experiment 09, 2026-09-01)

David reframed the ask: a single camera — any camera the user has laying
around — is a far more reasonable requirement than two; two-camera
triangulation is parked as a later refinement. Exp 09 reran the exp 07
protocol with 3D from one view via MotionBERT monocular lifting
(2D keypoints → H36M-17 → per-frame 3D; Apache-2.0, eval-only weights).
Each MoVi camera was treated as "the user's one camera": PG1 is ~frontal,
PG2 a ~90° side view.

Result: **frontal standing familiar poses survive — 1.6–5.3° calibrated MAE
on simple standing idles, inside the 5° budget and comparable to 2-cam.**
Seated poses fail (near-horizontal bones depth-flip by ~180°; part of the
chest/hip failure is a protocol artifact — MoVi has no genuinely recurring
poses, so the seated bias is calibrated in the wrong phase). The side view
fails before measurement: the pose gate's descriptor collapses when the hip
line is depth-dominated. Three mitigation lines emerged: use the drifted IMU
(±few °) to reject 180°-class flip hypotheses; a torso-scaled, view-robust
descriptor; and an explicit view-quality gate (the lifted pose itself says
how much of the lateral axis the camera actually sees). The real seated test
is the own-room recording — which now needs only one camera.
...
2. **Own-room dataset**: 1 RTSP camera (D36) + 11 trackers + Quest 3;
   recorder + ESP32 coded-blink beacon (D17). This is where recurring idle
   poses exist.
